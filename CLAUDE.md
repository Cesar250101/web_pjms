...
- `WebsiteSalePjms(WebsiteSale)` — extends the native `/shop` to support `?ribbon_tipo=<value>` filtering. The three overrides (`_get_search_domain`, `_shop_get_query_url_kwargs`, `_get_additional_extra_shop_values`) form a triple that **must stay in sync** — the domain filter, the URL preservation across pagination, and the value passed to the template. Forgetting one breaks pagination or filter persistence.
